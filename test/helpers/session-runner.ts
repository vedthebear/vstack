...
const HEARTBEAT_PATH = path.join(VSTACK_DEV_DIR, 'e2e-live.json'); // heartbeat stays global
const PROJECT_DIR = path.dirname(getProjectEvalDir()); // ~/.vstack/projects/$SLUG/
...
export async function runSkillTest(options: {
  ...
  if (browseErrors.length > 0 || exitReason !== 'success') {
    try {
      const failureDir = runDir || path.join(workingDirectory, '.vstack', 'test-transcripts');
