...
echo '{"skill":"canary","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.vstack/analytics/skill-usage.jsonl 2>/dev/null || true
...
```bash
eval "$(~/.claude/skills/vstack/bin/vstack-slug 2>/dev/null || echo "SLUG=unknown")"
mkdir -p .vstack/canary-reports
mkdir -p .vstack/canary-reports/baselines
mkdir -p .vstack/canary-reports/screenshots
```
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".vstack/canary-reports/baselines/<page-name>.png"
$B console --errors
$B perf
$B text
```
...
Save the baseline manifest to `.vstack/canary-reports/baseline.json`:
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".vstack/canary-reports/screenshots/pre-<page-name>.png"
$B console --errors
$B perf
```
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".vstack/canary-reports/screenshots/<page-name>-<check-number>.png"
$B console --errors
$B perf
```
...
Screenshots:   .vstack/canary-reports/screenshots/

VERDICT: [DEPLOY IS HEALTHY / DEPLOY HAS ISSUES — details above]
```

Save report to `.vstack/canary-reports/{date}-canary.md` and `.vstack/canary-reports/{date}-canary.json`.
