...
--verbose`). Results are persisted to `~/.vstack-dev/evals/` with auto-comparison
...
2. **If missing, AskUserQuestion** — let the user tell you or let vstack search the repo
...
When developing vstack, `.claude/skills/vstack` may be a symlink back to this
working directory (gitignored). This means skill changes are **live immediately** —
great for rapid iteration, risky during big refactors where half-written skills
could break other Claude Code sessions using vstack concurrently.

**Check once per session:** Run `ls -la .claude/skills/vstack` to see if it's a
symlink or a real copy. If it's a symlink to your working directory, be aware that:
- Template changes + `bun run gen:skill-docs` immediately affect all vstack invocations
- Breaking changes to SKILL.md.tmpl files can break concurrent vstack sessions
- During large refactors, remove the symlink (`rm .claude/skills/vstack`) so the
  global install at `~/.claude/skills/vstack/` is used instead

**Prefix setting:** Skill symlinks use either short names (`qa -> vstack/qa`) or
namespaced (`vstack-qa -> vstack/qa`), controlled by `skill_prefix` in
`~/.vstack/config.yaml`. When vendoring into a project, run `./setup` after
...
before going live (especially if the user is actively using vstack in other windows).
...
When estimating or discussing effort, always show both human-team and CC+vstack time:
...
Contributors can store long-range vision docs and design documents in `~/.vstack-dev/plans/`.
...
1. Push your branch
2. Fetch and reset in the skill directory: `cd ~/.claude/skills/vstack && git fetch origin && git reset --hard origin/main`
