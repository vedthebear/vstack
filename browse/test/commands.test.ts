...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/vstack-cli-');
    const execPath = path.join(root, '.claude/skills/vstack/browse/dist/browse');
    const serverPath = path.join(root, '.claude/skills/vstack/browse/src/server.ts');
