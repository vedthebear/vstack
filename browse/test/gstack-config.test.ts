/**
 * Tests for bin/vstack-config bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * VSTACK_STATE_DIR env override for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'vstack-config');
...
beforeEach(() => {
  stateDir = mkdtempSync(join(tmpdir(), 'vstack-config-test-'));
});
...
describe('vstack-config', () => {
  ...
  test('set creates state dir if missing', () => {
    ...
    const { exitCode } = run(['set', 'foo', 'bar'], { VSTACK_STATE_DIR: nestedDir });
    expect(exitCode).toBe(0);
