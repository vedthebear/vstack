...
description: |
  ...
  "how do I deploy with vstack", "add deploy config".
allowed-tools:
  - Bash
  - Read
  - Write
...
echo '{"skill":"setup-deploy","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.vstack/analytics/skill-usage.jsonl 2>/dev/null || true
...
# /setup-deploy — Configure Deployment for vstack
...
3. **How can vstack check if a deploy succeeded?**
