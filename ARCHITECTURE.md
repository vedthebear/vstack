...
This document explains **why** vstack is built the way it is. For setup and commands, see CLAUDE.md. For contributing, see CONTRIBUTING.md.

## The core idea

vstack gives Claude Code a persistent browser and a set of opinionated workflow skills. The browser is the hard part — everything else is Markdown.

The key insight: an AI agent interacting with a browser needs **sub-second latency** and **persistent state**. If every command cold-starts a browser, you're waiting 3-5 seconds per tool call. If the browser dies between commands, you lose cookies, tabs, and login sessions. So vstack runs a long-lived Chromium daemon that the CLI talks to over localhost HTTP.
...
1. **Compiled binaries.** `bun build --compile` produces a single ~58MB executable. No `node_modules` at runtime, no `npx`, no PATH configuration. The binary just runs. This matters because vstack installs into `~/.claude/skills/` where users don't expect to manage a Node.js project.
...
The server writes `.vstack/browse.json` (atomic write via tmp + rename, mode 0o600):
...
Cookies are the most sensitive data vstack handles. The design:

1. **Keychain access requires user approval.** First cookie import per browser triggers a macOS Keychain dialog. The user must click "Allow" or "Always Allow." vstack never silently accesses credentials.
...
3. **Database is read-only.** vstack copies the Chromium cookie DB to a temp file (to avoid SQLite lock conflicts with the running browser) and opens it read-only. It never modifies your real browser's cookie database.
...
```
Browser events → CircularBuffer (in-memory) → Async flush to .vstack/*.log
```
...
```bash
jq '.tests[] | select(.exit_reason == "timeout") | .last_tool_call' ~/.vstack-dev/evals/_partial-e2e.json
```
...
`eval:compare` diffs two eval runs. `eval:summary` aggregates stats across all runs in `~/.vstack-dev/evals/`.
