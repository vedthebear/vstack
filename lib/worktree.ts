...
function copyDirSync(src: string, dest: string): void {
  ...
  for (const entry of fs.readdirSync(src, { withFileTypes: true })) {
    // Skip symlinks to avoid infinite recursion (e.g., .claude/skills/vstack → repo root)
...
export class WorktreeManager {
  ...
  create(testName: string): string {
    ...
    const worktreeBase = path.join(this.repoRoot, '.vstack-worktrees', this.runId);
  ...
  cleanupAll(): void {
    ...
    const runDir = path.join(this.repoRoot, '.vstack-worktrees', this.runId);
    try {
  ...
  pruneStale(): void {
    try {
      ...
      const worktreeBase = path.join(this.repoRoot, '.vstack-worktrees');
