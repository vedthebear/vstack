...
version: 0.1.0
description: |
  Launch real Chrome controlled by vstack with the Side Panel extension auto-loaded.
...
echo '{"skill":"connect-chrome","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.vstack/analytics/skill-usage.jsonl 2>/dev/null || true
...
Connect Claude to a visible Chrome window with the vstack extension auto-loaded.
...
if [ -f "$(git rev-parse --show-toplevel 2>/dev/null)/.vstack/browse.json" ]; then
  _OLD_PID=$(cat "$(git rev-parse --show-toplevel)/.vstack/browse.json" 2>/dev/null | grep -o '"pid":[0-9]*' | grep -o '[0-9]*')
  [ -n "$_OLD_PID" ] && kill "$_OLD_PID" 2>/dev/null || true
  sleep 1
  [ -n "$_OLD_PID" ] && kill -9 "$_OLD_PID" 2>/dev/null || true
  rm -f "$(git rev-parse --show-toplevel)/.vstack/browse.json"
fi
# Clean Chromium profile locks (can persist after crashes)
_PROFILE_DIR="$HOME/.vstack/chromium-profile"
...
- The vstack Chrome extension auto-loaded via `launchPersistentContext`
...
The `connect` command auto-discovers the extension from the vstack install
...
```bash
cat "$(git rev-parse --show-toplevel 2>/dev/null)/.vstack/browse.json" 2>/dev/null | grep -o '"port":[0-9]*' | grep -o '[0-9]*'
```
...
_EXT_PATH=""
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
[ -n "$_ROOT" ] && [ -f "$_ROOT/.claude/skills/vstack/extension/manifest.json" ] && _EXT_PATH="$_ROOT/.claude/skills/vstack/extension"
[ -z "$_EXT_PATH" ] && [ -f "$HOME/.claude/skills/vstack/extension/manifest.json" ] && _EXT_PATH="$HOME/.claude/skills/vstack/extension"
...
> Chrome is launched with vstack control. You should see Playwright's Chromium
...
>    already show the vstack icon if the extension loaded successfully
> 2. Click the **puzzle piece** → find **vstack browse** → click the **pin icon**
> 3. Click the pinned **vstack icon** in the toolbar
...
> 2. Look for **"vstack browse"** — it should be listed and enabled
...
>
> If the Side Panel badge stays gray (disconnected), click the vstack icon
...
> - Run any vstack skill (`/qa`, `/design-review`, `/benchmark`) and watch
