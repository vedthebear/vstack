// vstack telemetry-ingest edge function
// Validates and inserts a batch of telemetry events.
// Called by bin/vstack-telemetry-sync.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const event of events) {
      // Required fields
      if (!event.ts || !event.vstack_version || !event.os || !event.outcome) {
      ...
      rows.push({
        ...
        vstack_version: String(event.vstack_version).slice(0, 20),
